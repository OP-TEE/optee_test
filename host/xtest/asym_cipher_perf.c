		  ...
		  uint32_t rsa_algo, int warmup, uint32_t l, uint32_t n)
{
	...
	fprintf(stderr, " [-t main_algo] [-m mode] [-n LOOP] [-r|--no-inited] [-d WIDTH_BITS]");
	...
	fprintf(stderr, "  -m MODE          Test asymmetric mode\n");
	fprintf(stderr, "                   ENC, DEC, SIGN, VERIFY, GENKEYPAIR\n");
...
{
	uint32_t cmd = TA_CRYPTO_PERF_CMD_ASYM_CIPHER_PREPARE_OBJ;
...
{
	uint32_t cmd = TA_CRYPTO_PERF_CMD_ASYM_CIPHER_PREPARE_HASH;
	TEEC_Operation op = TEEC_OPERATION_INITIALIZER;
	TEEC_Result res = TEEC_ERROR_GENERIC;
	uint32_t ret_origin = 0;
...
{
	uint32_t cmd = TA_CRYPTO_PERF_CMD_ASYM_CIPHER_PREPARE_KEYPAIR;
	TEEC_Operation op = TEEC_OPERATION_INITIALIZER;
	TEEC_Result res = TEEC_ERROR_GENERIC;
	uint32_t ret_origin = 0;
...
{
	uint32_t cmd = TA_CRYPTO_PERF_CMD_ASYM_CIPHER_PREPARE_ENC_SIGN;
	TEEC_Operation op = TEEC_OPERATION_INITIALIZER;
	TEEC_Result res = TEEC_ERROR_GENERIC;
	uint32_t ret_origin = 0;
...
static uint32_t get_curve_id(uint32_t width_bits)
{
	...
	case ECC_CURVE_521:
		...
		break;
	default:
		printf("ECC curve is not supported!\n");
		break;
	}
...
{
	TEEC_Operation op = TEEC_OPERATION_INITIALIZER;
	uint32_t cmd = TA_CRYPTO_PERF_CMD_ASYM_CIPHER_PROCESS;
	TEEC_Result res = TEEC_ERROR_GENERIC;
	...
	if (mode == MODE_DECRYPT && main_algo == RSA)
	...
	if (mode == MODE_GENKEYPAIR) {
		cmd = TA_CRYPTO_PERF_CMD_ASYM_CIPHER_PROCESS_GEN_KEYPAIR;
		op.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INPUT,
		...
		cmd = TA_CRYPTO_PERF_CMD_ASYM_CIPHER_PROCESS;
	...
	while (n-- > 0) {
		if (mode == MODE_ENCRYPT && main_algo == RSA) {
			/* Make sure the N > M */
			((unsigned char *)(in_shm.buffer))[0] = 0x00;
...
static int get_cipher_hash_len(uint32_t algo)
{
	switch (algo) {
...
static int get_sign_hash_len(uint32_t algo)
{
	switch (algo) {
...
static int check_rsa_cipher_params(uint32_t rsa_algo, int width_bits, int size)
{
	...
	if (rsa_algo == RSA_NOPAD) {
		...
		hash_len = get_cipher_hash_len(rsa_algo);
		if (hash_len == -1)
			return -1;
				 ...
				 int salt_len)
{
	int width_bytes = BITS_TO_BYTES(width_bits);
	int salt_temp = 0;
	int hash_len = get_sign_hash_len(rsa_algo);
...
static int get_rsa_cipher_mode(char *argv)
{
	int rsa_cipher_mode = RSA_NOPAD;

	if (!strcasecmp(argv, "RSAES_PKCS1_V1_5"))
		rsa_cipher_mode = RSAES_PKCS1_V1_5;
	else if (!strcasecmp(argv, "RSAES_PKCS1_OAEP_SHA1"))
		rsa_cipher_mode = RSAES_PKCS1_OAEP_SHA1;
	else if (!strcasecmp(argv, "RSAES_PKCS1_OAEP_SHA224"))
		rsa_cipher_mode = RSAES_PKCS1_OAEP_SHA224;
	else if (!strcasecmp(argv, "RSAES_PKCS1_OAEP_SHA256"))
		rsa_cipher_mode = RSAES_PKCS1_OAEP_SHA256;
	else if (!strcasecmp(argv, "RSAES_PKCS1_OAEP_SHA384"))
		rsa_cipher_mode = RSAES_PKCS1_OAEP_SHA384;
	else if (!strcasecmp(argv, "RSAES_PKCS1_OAEP_SHA512"))
		rsa_cipher_mode = RSAES_PKCS1_OAEP_SHA512;
	else {
		fprintf(stderr, "%s, invalid RSA cipher mode\n", argv);
		return -1;
	}

	return rsa_cipher_mode;
}

static int get_rsa_sign_mode(char *argv)
{
	int rsa_sign_mode = RSASSA_PKCS1_V1_5_SHA1;

	if (!strcasecmp(argv, "RSASSA_PKCS1_V1_5_SHA1"))
		rsa_sign_mode = RSASSA_PKCS1_V1_5_SHA1;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_V1_5_SHA224"))
		rsa_sign_mode = RSASSA_PKCS1_V1_5_SHA224;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_V1_5_SHA256"))
		rsa_sign_mode = RSASSA_PKCS1_V1_5_SHA256;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_V1_5_SHA384"))
		rsa_sign_mode = RSASSA_PKCS1_V1_5_SHA384;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_V1_5_SHA512"))
		rsa_sign_mode = RSASSA_PKCS1_V1_5_SHA512;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_PSS_MGF1_SHA1"))
		rsa_sign_mode = RSASSA_PKCS1_PSS_MGF1_SHA1;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_PSS_MGF1_SHA224"))
		rsa_sign_mode = RSASSA_PKCS1_PSS_MGF1_SHA224;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_PSS_MGF1_SHA256"))
		rsa_sign_mode = RSASSA_PKCS1_PSS_MGF1_SHA256;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_PSS_MGF1_SHA384"))
		rsa_sign_mode = RSASSA_PKCS1_PSS_MGF1_SHA384;
	else if (!strcasecmp(argv, "RSASSA_PKCS1_PSS_MGF1_SHA512"))
		rsa_sign_mode = RSASSA_PKCS1_PSS_MGF1_SHA512;
	else {
		fprintf(stderr, "%s, invalid RSA sign mode\n", argv);
		return -1;
	}

	return rsa_sign_mode;
}
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-t")) {
			...
			if (!strcasecmp(argv[i], "DH"))
				main_algo = DH;
			else if (!strcasecmp(argv[i], "RSA"))
				main_algo = RSA;
			else if (!strcasecmp(argv[i], "ECDSA"))
				main_algo = ECDSA;
			...
			NEXT_ARG(i);
			if (!strcasecmp(argv[i], "ENC"))
				mode = MODE_ENCRYPT;
			else if (!strcasecmp(argv[i], "DEC"))
				mode = MODE_DECRYPT;
			else if (!strcasecmp(argv[i], "SIGN"))
				mode = MODE_SIGN;
			else if (!strcasecmp(argv[i], "VERIFY"))
				mode = MODE_VERIFY;
			else if (!strcasecmp(argv[i], "KYEPAIR"))
			...
			if (mode == MODE_ENCRYPT || mode == MODE_DECRYPT)
				rsa_algo = get_rsa_cipher_mode(argv[i]);
			else if (mode == MODE_SIGN || mode == MODE_VERIFY)
				rsa_algo = get_rsa_sign_mode(argv[i]);
