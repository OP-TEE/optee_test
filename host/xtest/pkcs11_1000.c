 ...
 * success. Any base/derived key objects created are destroyed before
 * returning.
 */
static CK_RV hkdf_derive_and_get(ADBG_Case_t *c, CK_SESSION_HANDLE session,
				 ...
				 uint8_t *okm_out)
{
	...
	rv = C_DeriveKey(session, &mech, base_key, derived_template,
			 ARRAY_SIZE(derived_template), &derived_key);
	if (rv)
		goto out;

	rv = C_GetAttributeValue(session, derived_key, get_value,
...
static void xtest_pkcs11_test_1031(ADBG_Case_t *c)
{
	...
	Do_ADBG_BeginSubCase(c, "RFC 5869 A.1 - SHA-256, salt+info");
	{
		...
		rv = hkdf_derive_and_get(c, session, hkdf_rfc5869_a1_ikm,
	...
	Do_ADBG_BeginSubCase(c, "RFC 5869 A.2 - SHA-256, long inputs");
	{
		...
		rv = hkdf_derive_and_get(c, session, hkdf_rfc5869_a2_ikm,
	...
	Do_ADBG_BeginSubCase(c, "RFC 5869 A.3 - SHA-256, null salt, no info");
	{
		...
		rv = hkdf_derive_and_get(c, session, hkdf_rfc5869_a3_ikm,
	...
	Do_ADBG_BeginSubCase(c, "RFC 5869 A.1 - salt supplied as key object");
	{
		...
		if (ADBG_EXPECT_CK_OK(c, rv)) {
			...
			rv = hkdf_derive_and_get(c, session,
						 hkdf_rfc5869_a1_ikm,
	...
	Do_ADBG_BeginSubCase(c, "RFC 5869 A.1 - expand-only (PRK as base key)");
	{
		...
		rv = hkdf_derive_and_get(c, session, hkdf_rfc5869_a1_prk,
	...
	Do_ADBG_BeginSubCase(c, "RFC 5869 A.4 - SHA-1, salt+info");
	{
		...
		rv = hkdf_derive_and_get(c, session, hkdf_rfc5869_a4_ikm,
	...
	Do_ADBG_BeginSubCase(c, "Negative: base key without CKA_DERIVE");
	{
		...
		if (base_key != CK_INVALID_HANDLE)
			ADBG_EXPECT_CK_OK(c,
					  C_DestroyObject(session, base_key));
	}
	Do_ADBG_EndSubCase(c, NULL);
	...
	Do_ADBG_BeginSubCase(c, "RFC 5869 A.1 - private base key (User session)");
	{
		uint8_t okm[sizeof(hkdf_rfc5869_a1_okm)] = { 0 };
		CK_ATTRIBUTE base_template[] = {
			{ CKA_CLASS, &secret, sizeof(secret) },
		...
		if (ADBG_EXPECT_CK_OK(c, rv)) {
			rv = C_DeriveKey(session, &mech, base_key,
					 ...
					 &derived_key);
			if (ADBG_EXPECT_CK_OK(c, rv)) {
				rv = C_GetAttributeValue(session, derived_key,
							 get_value,
							 ARRAY_SIZE(get_value));
				if (ADBG_EXPECT_CK_OK(c, rv))
	...
	Do_ADBG_BeginSubCase(c, "RFC 5869 A.1 - private salt key (User session)");
	{
		uint8_t okm[sizeof(hkdf_rfc5869_a1_okm)] = { 0 };
		CK_ATTRIBUTE salt_template[] = {
			{ CKA_CLASS, &secret, sizeof(secret) },
