...
/*
 * Wycheproof ecdh_secp256r1_test.json tcId 315
 *
 * ECDH with a private key d=3. This private key serializes to a 1 byte bignum,
 * which used to trigger an assert in the CAAM driver code. While it is an
 * insecure key it is valid and should return the correct result.
 */
static void xtest_tee_test_4019(ADBG_Case_t *c)
{
	...
	static const uint8_t peer_public_key_x[] = {
		0x31, 0x02, 0x8f, 0x33, 0x77, 0xfc, 0x8f, 0x2b,
		0x19, 0x67, 0xed, 0xaa, 0xb9, 0x02, 0x13, 0xac,
		0xad, 0x0d, 0xa9, 0xf5, 0x08, 0x97, 0xf0, 0x8f,
		0x57, 0x53, 0x7f, 0x78, 0xf1, 0x16, 0x74, 0x47,
	};
	static const uint8_t peer_public_key_y[] = {
		0x43, 0xa1, 0x93, 0x01, 0x89, 0x36, 0x3b, 0xbd,
		0xe2, 0xac, 0x4c, 0xbd, 0x16, 0x49, 0xcd, 0xc6,
		0xf4, 0x51, 0xad, 0xd7, 0x1d, 0xd2, 0xf1, 0x6a,
		0x8a, 0x86, 0x7f, 0x2b, 0x17, 0xca, 0xa1, 0x6b,
	};

	static const uint8_t expected_secret[] = {
		0x85, 0xa0, 0xb5, 0x85, 0x19, 0xb2, 0x8e, 0x70,
		0xa6, 0x94, 0xec, 0x51, 0x98, 0xf7, 0x2c, 0x4b,
		0xfd, 0xab, 0xaa, 0x30, 0xa7, 0x0f, 0x71, 0x43,
		0xb5, 0xb1, 0xcd, 0x75, 0x36, 0xf7, 0x16, 0xca,
	};
	...
	res = ta_crypt_cmd_allocate_operation(c, &sess, &op, TEE_ALG_ECDH_P256,
					      TEE_MODE_DERIVE, 256);
	if (!ADBG_EXPECT_TEEC_SUCCESS(c, res))
		goto out;

	res = ta_crypt_cmd_allocate_transient_object(c, &sess,
	...
	res = ta_crypt_cmd_populate_transient_object(c, &sess, key_handle,
						     params, param_count);
	if (!ADBG_EXPECT_TEEC_SUCCESS(c, res))
		goto out;

	res = ta_crypt_cmd_set_operation_key(c, &sess, op, key_handle);
	if (!ADBG_EXPECT_TEEC_SUCCESS(c, res))
		goto out;
	...
	param_count = 0;
	xtest_add_attr(&param_count, params, TEE_ATTR_ECC_PUBLIC_VALUE_X,
		       peer_public_key_x, sizeof(peer_public_key_x));
	xtest_add_attr(&param_count, params, TEE_ATTR_ECC_PUBLIC_VALUE_Y,
		       peer_public_key_y, sizeof(peer_public_key_y));

	res = ta_crypt_cmd_derive_key(c, &sess, op, sv_handle, params,
				      param_count);
	if (!ADBG_EXPECT_TEEC_SUCCESS(c, res))
		goto out;

	res = ta_crypt_cmd_get_object_buffer_attribute(c, &sess, sv_handle,
						       TEE_ATTR_SECRET_VALUE,
						       out_buf, &out_size);
	if (!ADBG_EXPECT_TEEC_SUCCESS(c, res))
		goto out;

	ADBG_EXPECT_BUFFER(c, expected_secret, sizeof(expected_secret),
			   out_buf, out_size);

out:
	if (op != TEE_HANDLE_NULL)
		ta_crypt_cmd_free_operation(c, &sess, op);
	if (key_handle != TEE_HANDLE_NULL)
		ta_crypt_cmd_free_transient_object(c, &sess, key_handle);
	if (sv_handle != TEE_HANDLE_NULL)
		ta_crypt_cmd_free_transient_object(c, &sess, sv_handle);
	TEEC_CloseSession(&sess);
}

ADBG_CASE_DEFINE(regression, 4019, xtest_tee_test_4019,
		 "ECDH with 1-byte private key");

/*
 * ECDSA sign with 1-byte private key
 *
 * Same short-key deserialization bug as 4019, but via the ECDSA signing path.
 */
static void xtest_tee_test_4020(ADBG_Case_t *c)
{
	...
	res = ta_crypt_cmd_allocate_operation(c, &sess, &op,
					      TEE_ALG_ECDSA_P256,
					      TEE_MODE_SIGN, 256);
	if (!ADBG_EXPECT_TEEC_SUCCESS(c, res))
		goto out;
	...
	res = ta_crypt_cmd_asymmetric_sign(c, &sess, op, NULL, 0, digest,
					   sizeof(digest), sig_buf, &sig_len);
	if (!ADBG_EXPECT_TEEC_SUCCESS(c, res))
		goto out;
	...
	res = ta_crypt_cmd_asymmetric_verify(c, &sess, op, NULL, 0, digest,
					     sizeof(digest), sig_buf,
					     sig_len);
	ADBG_EXPECT_TEEC_SUCCESS(c, res);
...
/*
 * DSA sign with 1-byte private key
 *
 * Same short-key deserialization bug as 4019, but via the DSA signing path.
 */
static void xtest_tee_test_4021(ADBG_Case_t *c)
{
	static const uint8_t dsa_p[] = {
		,
	};
	static const uint8_t dsa_q[] = {
		0xba, 0xf6, 0x96, 0xa6, 0x85, 0x78, 0xf7, 0xdf,
		0xde, 0xe7, 0xfa, 0x67, 0xc9, 0x77, 0xc7, 0x85,
		0xef, 0x32, 0xb2, 0x33, 0xba, 0xe5, 0x80, 0xc0,
		0xbc, 0xd5, 0x69, 0x5d,
	};
	static const uint8_t dsa_g[] = {
		,
	};
	static const uint8_t dsa_y[] = {
		,
	};
	static const uint8_t dsa_x[] = {
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x03,
	};
	static const uint8_t digest[28] = { };

	TEE_ObjectHandle key_handle = TEE_HANDLE_NULL;
	TEE_OperationHandle op = TEE_HANDLE_NULL;
	TEEC_Result res = TEEC_ERROR_GENERIC;
	TEE_Attribute params[5] = { };
	TEEC_Session sess = { };
	size_t param_count = 0;
	uint32_t ret_orig = 0;
	...
	xtest_add_attr(&param_count, params, TEE_ATTR_DSA_BASE,
		       dsa_g, sizeof(dsa_g));
	xtest_add_attr(&param_count, params, TEE_ATTR_DSA_PUBLIC_VALUE,
		       dsa_y, sizeof(dsa_y));
	xtest_add_attr(&param_count, params, TEE_ATTR_DSA_PRIVATE_VALUE,
		       dsa_x, sizeof(dsa_x));
...
/*
 * DH derive with 1-byte private key
 *
 * Same short-key deserialization bug as 4019, but via the DH path.
 */
static void xtest_tee_test_4022(ADBG_Case_t *c)
{
	...
	static const uint8_t dh_g[] = { 0x02 };
	static const uint8_t dh_y[] = {
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	static const uint8_t dh_peer_y[] = {
		,
	};

	static const uint8_t expected_secret[] = {
		,
	};
