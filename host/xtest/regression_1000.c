...
/*
 * regression_1033
 * Depends on kernel commit "tee: add support for application-based session login methods"
...
static void xtest_tee_test_1033(ADBG_Case_t *c)
{
	...
	if (readlink("/proc/self/exe", app, PATH_MAX) < 0) {
		Do_ADBG_Log(" - 1033 -   skip test, "
			    "/proc/self/exe not readable");
		return;
	}

	snprintf(uuid_name, TEE_UUID_NS_NAME_SIZE + 4, "app=%s", app);
...
out:
	...
	UNUSED(c);
	/* xtest_uuid_v5() depends on OpenSSL */
	Do_ADBG_Log("OpenSSL not available, skipping test 1033");
#endif
}
...
static void xtest_tee_test_1034(ADBG_Case_t *c)
{
	...
	if (readlink("/proc/self/exe", app, PATH_MAX) < 0) {
		Do_ADBG_Log(" - 1034 -   skip test, "
			    "/proc/self/exe not readable");
		return;
	}

	snprintf(uuid_name, TEE_UUID_NS_NAME_SIZE + 16, "uid=%x:app=%s", geteuid(), app);
	uuid_name[TEE_UUID_NS_NAME_SIZE] = 0;
...
out:
	...
	UNUSED(c);
	/* xtest_uuid_v5() depends on OpenSSL */
	Do_ADBG_Log("OpenSSL not available, skipping test 1034");
#endif
}
...
static void xtest_tee_test_1035(ADBG_Case_t *c)
{
	...
	if (readlink("/proc/self/exe", app, PATH_MAX) < 0) {
		Do_ADBG_Log(" - 1035 -   skip test, "
			    "/proc/self/exe not readable");
		return;
	}

	snprintf(uuid_name, TEE_UUID_NS_NAME_SIZE + 16, "gid=%x:app=%s", group, app);
	uuid_name[TEE_UUID_NS_NAME_SIZE] = 0;
